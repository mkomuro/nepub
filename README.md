...
Web から取得したタイトルや作者名をそのまま表紙に反映させたくない場合は、任意のタイトル・作者名を指定して JPEG 表紙画像を生成できます。

タイトル文字列に半角の `|` を含めると、その位置で改行され、各行は表紙画像の横方向中央に揃えて描画されます。`|` が含まれていない場合は、自動折り返しされます。

以下に、上記のサンプルと同じ表紙画像を生成する際のコマンド例を記載します。
